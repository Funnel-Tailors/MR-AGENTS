...
description: "Use this agent when you need to validate application scalability, simulate high-traffic scenarios, identify performance bottlenecks, or stress-test systems under load. This includes pre-launch capacity planning, identifying breaking points, optimizing response times under concurrent load, and ensuring infrastructure can handle expected (and unexpected) traffic spikes.\\n\\nExamples:\\n\\n<example>\\nContext: User has just deployed a new API endpoint and wants to ensure it can handle production traffic.\\nuser: \"I just finished the new checkout API endpoint. Can you make sure it can handle our expected Black Friday traffic?\"\\nassistant: \"I'll use the load-test-engineer agent to simulate high-traffic conditions against your checkout endpoint and identify any bottlenecks before they become problems in production.\"\\n<Task tool call to load-test-engineer agent>\\n</example>\\n\\n<example>\\nContext: User notices their application is slow but isn't sure where the bottleneck is.\\nuser: \"Our app gets really sluggish when we have more than 100 concurrent users. Can you figure out why?\"\\nassistant: \"Let me launch the load-test-engineer agent to systematically stress test your application, identify the breaking point, and pinpoint exactly which components are causing the degradation.\"\\n<Task tool call to load-test-engineer agent>\\n</example>\\n\\n<example>\\nContext: User is preparing for a product launch and wants to validate their infrastructure.\\nuser: \"We're launching next week and expecting 50,000 users in the first hour. I need to know if we'll survive.\"\\nassistant: \"I'll use the load-test-engineer agent to simulate your expected launch traffic and beyond, identify any infrastructure weaknesses, and provide specific recommendations to ensure a smooth launch.\"\\n<Task tool call to load-test-engineer agent>\\n</example>\\n\\n<example>\\nContext: After implementing performance fixes, user wants to verify improvements.\\nuser: \"I've optimized the database queries you identified. Can you run the load tests again?\"\\nassistant: \"I'll use the load-test-engineer agent to re-run the load tests and compare the results against our baseline to quantify the improvements and identify any remaining bottlenecks.\"\\n<Task tool call to load-test-engineer agent>\\n</example>"
model: sonnet
mode: plan
